...
export function CandidateCard({ candidate, isSelected, onSelectChange }: CandidateCardProps) {
  ...
  return (
    <TooltipProvider>
      <div className="group relative flex items-center gap-4 p-4 bg-card rounded-lg card-tactical">
        {/* Checkbox */}
        <Checkbox
          checked={isSelected}
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {/* Short ID - Tactical styling */}
            {candidate.shortId && (
              <span className="text-tactical text-gold-400">
                #{candidate.shortId}
              </span>
            )}

            {/* Signal Light - Availability Status */}
            <SignalLight status={getSignalStatus(candidate.availabilityStatus)} size="md" />
          ...
          {/* Row 2: Role (Tactical), Experience, Location */}
          <div className="flex items-center gap-2 text-sm text-muted-foreground flex-wrap">
            <span className="text-tactical text-gold-400">
              {candidate.primaryRole || 'Ikke spesifisert'}
            </span>
            {candidate.secondaryRoles?.length > 0 && (
              <span className="text-xs text-muted-foreground">+{candidate.secondaryRoles.length}</span>
            )}
